...
- IoTデバイスを手元に用意してください。
    ...
    - IoTセンサー
        ...
        |製品名|製品説明|サンプルコード|
        |-----|-------|-----------|
        |[温湿度センサー](http://wiki.seeedstudio.com/Grove-TemperatureAndHumidity_Sensor/)|湿度と温度を取得する|[サンプル](./device/Hello-TempHumi/Hello-TempHumi.ino)|
        |[距離センサー](http://wiki.seeedstudio.com/Grove-Ultrasonic_Ranger/)|センサーからの距離を取得する。取得可能な範囲は2-350cmで、取得単位は、cm|[サンプル](./device/Hello-Ultrasonic/Hello-Ultrasonic.ino)|
        |[加速度センサー](http://wiki.seeedstudio.com/Grove-3-Axis_Digital_Accelerometer-1.5g/)|X軸、Y軸、Z軸の加速度を取得する。取得可能な数値は、最大±1.5gまで|[サンプル](./device/Hello-3Axis/Hello-3Axis.ino)|
        |[ボタンスイッチ](http://wiki.seeedstudio.com/Grove-Button/)|ボタンの状態を取得する|[サンプル](./device/Hello-Button/Hello-Button.ino)|
        |[磁気スイッチ](http://wiki.seeedstudio.com/Grove-Magnetic_Switch/)|磁石が近づいているかを検知する|[サンプル](./device/Hello-MagSW/Hello-MagSW.ino)|

- Wio LTEの開発環境の構築を行います。手順は各OSごとに異なりますので、手順に従い準備をお願いします。
    - [MACの場合](https://soracom.github.io/jp-training/WioLTE/0/prepare-mac)
    - [Windowsの場合](https://soracom.github.io/jp-training/WioLTE/0/prepare-win)
...
- [Step0](#Step0:-マイコンモジュールの動作確認): LEDの点灯（開発環境の動作確認）
...
1. PCにArduinoを接続したあと、左上のチェックマークをクリックします。「ボードの書き込みが完了しました。」と表示され、ArduinoのLEDが点灯したら、セットアップは完了です。
...
1. 温湿度センサーから値を取得する[サンプルコード](./device-sample/Hello-TempHumi/Hello-TempHumi.ino)をArduino IDEで開きます。デバイスへ書き込み、シリアルモニタから、温度、湿度が表示されていることを確認してください。
    ...
    - シリアルモニタを開くには、ツール -> シリアルポートから、Arduinoを接続したポートを選択したあと、ツール -> シリアルポート を選択してください。
...
1. 次に、SORACOM Harvestの動作確認を行います。SORACOM Harvestの1秒ごとにカウントアップしたデータをSoracom Harvestに送信する[サンプルコード](./device-sample/Hello-SORACOM-harvest/Hello-SORACOM-harvest.ino)を用意していますので、Arduino IDEで開いてください。デバイスへ書き込み、シリアルモニタに以下のようなログが表示されることを確認してください。

    <img src="./image/step1-5.png" width="640">

1. 最後に、送信されたデータを確認します。SORACOMコンソールのSIMカード一覧より、使用しているSIMカードの左のチェックボックスを選択し、操作 -> データを確認を選択します。以下のように、グラフが表示されていることを確認してください。
...
### Lambda関数の作成
1. AWSコンソールへログインし、Lmabda -> 関数 -> 関数の作成を選択してください。
...
### API Gatewayの設定
1. AWSコンソールへログインし、API Gateway新規作成外面を開き、以下のように入力し、APIの作成を選択してください。
    - プロトコルを選択する
        - REST
    - 新しいAPIの作成
        - 新しいAPI
...
1. API GatewayとLambdaの動作確認を行います。以下のコマンドを実行し、期待通りのレスポンスが得られることを確認してください。
    ...
    ```
    $ curl -X POST -H "Content-Type:application:json" https://wmv387e4re.execute-api.ap-northeast-1.amazonaws.com/dev
...
1. 関連付けられたAPIステージ画面が表示されます。先ほど作成したAPIステージの紐付けを行います。以下の通り選択し、次へを選択してください。
    - API: twitter-notice-api{チーム番号}
    - ステージ: dev

    <img src="./image/step2-15.png" width="640">

1. 次に使用するAPIキーを選択する画面が表示されます。先ほど作成したAPIキーを入力し、チェックマークを
選択してください。

    <img src="./image/step2-16.png" width="640">

1. 作成したAPI -> リソース -> POST -> メソッドリクエストを選択してください。
...
- APIキーをヘッダーに付与する場合
    ```
    $ curl -X POST -H "Content-Type:application:json" -H "x-api-key:{作成したAPIキー}" https://wmv387e4re.execute-api.ap-northeast-1.amazonaws.com/dev

    {"statusCode": 200, "body": "\"Hello from Lambda!\""}%
   ```

### Tweet機能をLambda関数に実装
1. AWSコンソールへログインし、Lmabda -> 関数 -> twitter-notice-function{チーム番号}(先ほど作成した関数)を選択してください。
...
Step3では、温湿度センサーから取得した温度・湿度をSORACOM Beamを利用し、MQTT
でクラウドサービスへ送信します。クラウドサービスとしてAWSのAWS IoT, Lambdaを利用し、温度・湿度に応じた家電制御機能を作成します。
...
1. 画面左のACTを選択し、右上の作成を選択してください。ルールの作成画面が表示されますので、以下の通り入力してください。
...
### [課題3] 距離に応じて、家電操作を行う
1. step3のこれまでの手順で作成したコードは、センサー情報が送られるたびに家電を操作するようになっています。
各自でLambdaコードのTODO部分を修正し、距離に応じて家電操作を実装してください。
